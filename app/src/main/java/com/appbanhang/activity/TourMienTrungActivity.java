...
import com.appbanhang.adapter.TourMienTrungAdapter;
...
public class TourMienTrungActivity extends AppCompatActivity {

    Toolbar toolbarxstd;
    ListView listViewxstd;
    TourMienTrungAdapter xeSoTuDongAdapter;
    ArrayList<SanPham> mangxstd;
    int idxstd = 0;
//    int page = 1;
    protected void onCreate(Bundle savedInstanceState){
        super.onCreate ( savedInstanceState );
        setContentView ( R.layout.activity_tourmientrung);

        AnhXa();
        listViewxstd.setOnItemClickListener ( new AdapterView.OnItemClickListener () {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                Intent intent = new Intent ( TourMienTrungActivity.this, ChiTietActivity.class );
                intent.putExtra ( "chitietsanpham", mangxstd.get ( position ) );
                startActivity ( intent );
            }
        } );
        if(CheckConnection.haveNetworkConnection ( TourMienTrungActivity.this )){
            GetIDLoaiSanPham();
            ActionToolbar();
            //GetData(page);
            Getdata2();
            ...
            CheckConnection.showToast_Short ( TourMienTrungActivity.this ,"Bạn hãy kiểm tra lại  kết nối" );
        }

    }

    private void Getdata2() {
            final RequestQueue request = Volley.newRequestQueue(TourMienTrungActivity.this);
            JsonArrayRequest json = new JsonArrayRequest(Server.duongdanxesotudong,new Response.Listener<JSONArray>() {
                ...
                public void onErrorResponse(VolleyError error) {
                    ...
                    Toast.makeText(TourMienTrungActivity.this, error.toString(), Toast.LENGTH_SHORT).show();
                }
            });
            request.add(json);
        }
    ...
    private void GetData(int Page) {
        RequestQueue requestQueue = Volley.newRequestQueue ( TourMienTrungActivity.this  );
    ...
    private void AnhXa() {
        ...
        xeSoTuDongAdapter = new TourMienTrungAdapter( getApplicationContext (),mangxstd );

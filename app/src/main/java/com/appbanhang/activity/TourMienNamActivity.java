...
import com.appbanhang.adapter.TourMienNamAdapter;
...
public class TourMienNamActivity extends AppCompatActivity {
    Toolbar toolbarxctd;
    ListView listViewxctd;
    TourMienNamAdapter xeConTuDongAdapter;
    ArrayList<SanPham> mangxctd;
    int idxctd = 0;
    protected void onCreate(Bundle savedInstanceState){
        super.onCreate ( savedInstanceState );
        setContentView ( R.layout.activity_tourmiennam);
        AnhXa();
        listViewxctd.setOnItemClickListener ( new AdapterView.OnItemClickListener () {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                Intent intent = new Intent ( TourMienNamActivity.this, ChiTietActivity.class );
                intent.putExtra ( "chitietsanpham", mangxctd.get ( position ) );
                startActivity ( intent );
            }
        } );
        if(CheckConnection.haveNetworkConnection ( TourMienNamActivity.this )){
            GetIDLoaiSanPham();
            ActionToolbar();
            //GetData(page);
            Getdata2();
            ...
            CheckConnection.showToast_Short ( TourMienNamActivity.this ,"Bạn hãy kiểm tra lại  kết nối" );
        }

    }

    private void Getdata2() {
        final RequestQueue request = Volley.newRequestQueue(TourMienNamActivity.this);
        JsonArrayRequest json = new JsonArrayRequest( Server.duongdanxecontudong,new Response.Listener<JSONArray>() {
            ...
            public void onErrorResponse(VolleyError error) {
                ...
                Toast.makeText(TourMienNamActivity.this, error.toString(), Toast.LENGTH_SHORT).show();
            }
        });
        request.add(json);
    }
    ...
    private void AnhXa() {
        ...
        xeConTuDongAdapter = new TourMienNamAdapter( getApplicationContext (),mangxctd );
